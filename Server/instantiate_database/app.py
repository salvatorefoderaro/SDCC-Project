...
def instantiateDatabase():
    ...
    cursor.execute("INSERT into devicesGroups (groupName, p1, p2, p3) values (\'default\', 0, 0, 0) ON DUPLICATE KEY UPDATE p1 = 0, p2 = 0, p3 = 0")
    db.commit()
    ...
    db.commit()
    cursor.execute("CREATE TABLE IF NOT EXISTS water_level (id INT, water_L float, lettura DATETIME, PRIMARY KEY (id), FOREIGN KEY (id) REFERENCES devices(id))")
    db.commit()
    cursor.close()
    db.close()
